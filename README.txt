################################################
RSA Chat Messenger
Topics in Network Security MiniProject
Made By:
Alon Arbel 205730849
Yonatan Shimoni 318543824
Eldad Sasportas 208546465
################################################
Before running the project make sure
that JVM and JAVA 19 JDK is installed properly.
The code was written on Windows machine.
################################################
To run the project:
1.Run the ChatServer.java First using JVM and the 
Server will start listening to port 9000
2.Run clients with ChatClient.java using JVM
and type a name for each client.
*Note that when starting a client you will be asked if you
want to use DEBUG mode, so you could see the 
messeges encrypted and decrypted using the private
and public keys.*
3.Write messeges using the client terminal to 
commuincate with others in the room.
4.To disconnect a client type "BYE".
################################################


